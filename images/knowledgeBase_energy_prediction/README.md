...
1. Build docker image from Dockerfile specified.
    ...
    ```
    docker build --tag knowledgebase_energy_prediction:latest .
    ```

1. Have a look on the image created.

    ```
    docker run -it --rm knowledgebase_energy_prediction:latest sh
    ```
...
1. Test your container, e.g. by executing a shell.
    ...
    ```
    docker exec -it knowledgebase_energy_prediction sh
    ```
...
1. To push the image to `https://hub.docker.com/` the following command can be used.
    ...
    ```
    docker image push anteab/knowledgebase_energy_prediction:latest
    ```
